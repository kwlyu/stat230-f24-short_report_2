...
ggpairs(data = wine_dat, columns = c("alcohol", "sulphates", "pH", "residual.sugar", "quality")) # for interested vars
...
## Interaction Model
wine_lm_interaction <- lm(quality ~ alcohol + sulphates + residual.sugar + pH*color, data = wine_dat)
...
## transformed model
wine_lm_interaction_tran <- lm(quality ~ alcohol + log(sulphates) + log(residual.sugar) + pH*color, data = wine_dat)
...
## interaction vs base, untransformed
anova(wine_lm_base, wine_lm_interaction)

## interaction vs base, transformed
anova(wine_lm_base, wine_lm_interaction_tran)
...
vif(wine_lm_base)
vif(lm(quality ~ alcohol + sulphates + residual.sugar + pH + color, data = wine_dat)) # Might be this one; the base model doesn't have a color term
vif(lm(quality ~ alcohol + log(sulphates) + log(residual.sugar) + pH + color, data = wine_dat)) # or the transformed one
```
...
upper_model <- lm(quality ~ (pH + alcohol + log(residual.sugar) + color + log(sulphates) + fixed.acidity + volatile.acidity + citric.acid + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density)^2,
                 data = wine_dat)
